...
\begin{itemize}
    ...
    \includegraphics[width=\textwidth]{masked_self_attention.drawio.png}
...
\begin{figure}[h] 
    ...
    \caption{GPT-3 and GPT-J architectures compared}\label{figure:GPT-architectures}
\end{figure}    


Compared to GPT-3, GPT-J has two minor architectural differences\cite{gpt-j} ( shown in \autoref{figure:GPT-architectures}):
\begin{itemize}
...
RoPE instead proposes to incorporate the relative position information by multiplyng the context representation with the sinusoidal functions instead if directly adding them. \\
If we define 
